...
| Magnetic canvas layout | Edit mode presents visuals as bounded blocks on a 12-column grid. Use the grip handle to move a block; it snaps to the grid and finds the nearest free slot on drop. Numeric layout controls remain available for precise placement. | Open a report, choose Edit, drag a visual by its grip, drop it beside another visual, and Save. Automated coverage: `core/src/layout.spec.ts`. |

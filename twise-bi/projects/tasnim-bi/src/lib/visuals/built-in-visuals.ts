import { BiVisualType, DataRole } from '../core/visual-registry';
...
const chart = () => import('./chart-visual.component').then((m) => m.ChartVisualComponent);

/** The visuals every host gets from provideBi(). Icons are 24×24 stroke paths. */
export const BUILT_IN_VISUALS: BiVisualType[] = [
  ...
  { type: 'donut', label: 'Donut chart', icon: 'M12 3a9 9 0 1 0 9 9h-5a4 4 0 1 1-4-4V3Z', dataKind: 'aggregate', roles: [category('Legend'), values('Values', 1)], defaultInteraction: 'highlight', loadComponent: chart },
  {
    ...
    loadComponent: () => import('./card-visual.component').then((m) => m.CardVisualComponent),
  },
  {
    ...
    loadComponent: () => import('./table-visual.component').then((m) => m.TableVisualComponent),
  },
  {
    ...
    loadComponent: () => import('./slicer-visual.component').then((m) => m.SlicerVisualComponent),
  },
];
